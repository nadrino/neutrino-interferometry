...
P = osc.probability(L_km=295, E_GeV=np.linspace(0.2,2,50), alpha=None, beta=None)
np.testing.assert_allclose(P.sum(axis=-2), 1.0, atol=1e-12)


from nu_waves.backends import make_torch_mps_backend
torch_backend = make_torch_mps_backend(seed=123, use_complex64=True)  # MPS if available
